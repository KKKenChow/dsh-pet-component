import type { RefOrValue } from '../../../src/utils/react'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { toValue } from '../../../src/utils/react'

/** 指针类型（VueUse `useDraggable` 的 `PointerType`）。 */
...
export function useDraggable(
  ...
  useEffect(() => {
    ...
    // 规则只看得到「本行 addEventListener」，认不出下面 cleanup 里按同一数组逐个
    // removeEventListener 的写法。
    // eslint-disable-next-line react/web-api-no-leaked-event-listener -- cleanup 按同一 listeners 数组逐个卸载
