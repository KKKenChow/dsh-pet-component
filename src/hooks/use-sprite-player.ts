...
export function useSpritePlayer(options: UseSpritePlayerOptions): void {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      if (reducedMotion) {
        if (!loop)
          schedule(finish, interval)
        return
      }

      if (index >= lastIndex) {
        if (!loop) {
          // 停末帧一个 interval 再回落，避免硬切
          schedule(finish, interval)
          return
        }
        indexRef.current = 0
  ...
  }, [columns, elementRef, frames, interval, lookIndex, loop, reducedMotion, revision, row, rows])
}
